...
class SlashChecking(commands.Cog):
    def __init__(self, bc):
        self.bc = bc

    @slash.command(description="Diagnose a command to see why it may not be working!")
    ...
    @slash.command(description="Check out the hypixel stats of a user")
...
""".format(data["recentgames"][0]["gameType"],mode,data["recentgames"][0]["map"]))
    ...
    @slash.group(invoke_without_command=True)
...
""".format(f"**{nick}**",f"```{activity}```",f"**{new_status}**",f"**{user.bot}**",booster,f"**{user.created_at.strftime('%a, %#d %B %Y, %I:%M %p')}**",f"**{user.joined_at.strftime('%a, %#d %B %Y, %I:%M %p')}**"))
    ...
    @slash.command(description="See what modroles are set in your server")
