...
class ReactionRoles(commands.Cog):
    ...
    async def rebuild_role_embed(self, guild_id):
        data = await self.bc.config.find(guild_id)
        channel_id = data["channel_id"]
        message_id = data["message_id"]

        guild = await self.bc.fetch_guild(guild_id)
        channel = await self.bc.fetch_channel(channel_id)
        ...
        desc = ""
        reaction_roles = await self.bc.reaction_roles.get_all()
    ...
    @commands.group(
        aliases=['rr'], usage="", invoke_without_command=True
    )
    @commands.guild_only()
    async def reactionroles(self, ctx):
        await ctx.invoke(self.bc.get_command("help"), entity="reactionroles")
    ...
    async def rr_toggle(self, ctx):
        ...
        data = await self.bc.config.find(ctx.guild.id)
        data["is_enabled"] = not data["is_enabled"]
        await self.bc.config.upsert(data)
    ...
    async def rr_remove(self, ctx, emoji: typing.Union[discord.Emoji, str]):
        ...
        await self.bc.reaction_roles.delete(emoji)
    ...
    async def on_raw_reaction_add(self, payload):
        data = await self.bc.config.find(payload.guild_id)
        ...
        guild = await self.bc.fetch_guild(payload.guild_id)

        emoji_data = await self.bc.reaction_roles.find(str(payload.emoji))
